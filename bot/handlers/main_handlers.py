...
from bot.api import create_client

async def register(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(RegisterStates.full_name_input)
    await call.message.answer("Введите ФИО клиента")

async def control_table(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(TrackStates.code_input)
    await call.message.answer("Введите ваш клиентский код:")

async def ransom(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(RansomStates.code_input)
    await call.message.answer("Введите ваш клиентский код:")

async def fill_address(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer("Выберите маркетплейс по которому хотите получить инструкцию", reply_markup=fill_address_kb())

async def back_to_main(call: types.CallbackQuery, state: FSMContext):
    await state.set_state("*")
    await state.reset_state()
    await call.message.answer("Вы на главной странице бота. Воспользуйтесь меню ниже:", reply_markup=main_keyboard())
...
async def full_name_input(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите контактный номер телефона клиента (+7915789504, 8915789504)")
...
async def is_correct(call: types.CallbackQuery, state: FSMContext):
    ...
    registartion_text = "Спасибо за регистрацию, мы рады с вами сотрудничать ❤️\n\n"\
                    ...
                    "Комментарий : <code>广东省佛山市南海区里水镇文教路18号 柏华工 业园D17润泽国际-TX-VZ3276</code>"
...
async def is_not_correct(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(RegisterStates.full_name_input)
    await call.message.answer("Скрипт сброшен! Для начала введите свои данные ФИО или данные клиента:")
